...
function buildVolumeMounts(
  ...
  // Gmail OAuth credentials (read-write: MCP server may refresh tokens)
  ...
  // Second Gmail account credentials
...
async function buildContainerArgs(
  ...
  // Pass GH_TOKEN so the container can use the gh CLI
  const ghToken = process.env.GH_TOKEN || readEnvFile(['GH_TOKEN']).GH_TOKEN;
  if (ghToken) {
  ...
  const sourcebotUrl =
    process.env.SOURCEBOT_MCP_URL ||
    readEnvFile(['SOURCEBOT_MCP_URL']).SOURCEBOT_MCP_URL;
  if (sourcebotUrl) {
    args.push('-e', `SOURCEBOT_MCP_URL=${sourcebotUrl}`);
    const sourcebotToken =
      process.env.SOURCEBOT_MCP_TOKEN ||
      readEnvFile(['SOURCEBOT_MCP_TOKEN']).SOURCEBOT_MCP_TOKEN;
    if (sourcebotToken) {
